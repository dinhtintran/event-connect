...
import 'package:event_connect/core/constants/app_roles.dart';
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _submit() async {
    ...
    if ((status == 200 || status == 201) && body['access'] != null) {
      // Route by role based on backend values
              ...
              Center(
                child: RichText(
                  text: TextSpan(text: 'Chưa có tài khoản? ', style: TextStyle(color: Colors.grey[700]), children: [
                    TextSpan(text: 'Đăng ký', style: TextStyle(color: theme.colorScheme.primary), recognizer: TapGestureRecognizer()..onTap = () => Navigator.of(context).pushNamed(AppRoutes.register)),
                  ]),
                ),
              ),
            ],
