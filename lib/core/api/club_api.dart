...
class ClubApi {
  ...
  /// GET /api/clubs/{club_id}/statistics/ - Lấy thống kê đã tổng hợp của CLB
  Future<Map<String, dynamic>> getClubStatistics(
    ...
    _dbg('GET /api/clubs/$clubId/statistics/');
    try {
      final queryParams = <String, dynamic>{};
      ...
      final res = await dio.get('/api/clubs/$clubId/statistics/', queryParameters: queryParams);
      _dbg('response ${res.statusCode} ${res.requestOptions.uri}');
      return {'status': res.statusCode, 'body': res.data};
    } on DioException catch (e) {
      _dbg('DioException: type=${e.type} status=${e.response?.statusCode} error=${e.message}');
  ...
  /// GET /api/clubs/{club_id}/statistics/raw-events/ - Lấy danh sách sự kiện thô phục vụ phân tích
  Future<Map<String, dynamic>> getClubStatisticsRawEvents(
    ...
    _dbg('GET /api/clubs/$clubId/statistics/raw-events/');
    try {
      final queryParams = <String, dynamic>{};
      if (rangeDays != null) queryParams['range_days'] = rangeDays;
      ...
      final res = await dio.get('/api/clubs/$clubId/statistics/raw-events/', queryParameters: queryParams);
      _dbg('response ${res.statusCode} ${res.requestOptions.uri}');
      return {'status': res.statusCode, 'body': res.data};
    } on DioException catch (e) {
      _dbg('DioException: type=${e.type} status=${e.response?.statusCode} error=${e.message}');
